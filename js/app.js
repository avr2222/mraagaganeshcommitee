...
let custodyDetailCurrent = null;
function openCustodyDetailModal(c){
  custodyDetailCurrent = c;
...
// A per-person cash statement -- everything that made up their current
// balance, in a printable/shareable form (e.g. to send to a committee
// member so they can see exactly what they've collected/spent and what
// they still owe or are owed).
function buildCustodyDetailReportHTML(c){
  const itemRows = (items, cls) => items.map(x=>`
    <tr><td>${x.dateFmt}</td><td>${escapeHtml(x.label)}</td><td class="num ${cls}">${x.amountFmt}</td></tr>
  `).join('') || `<tr><td colspan="3">None recorded.</td></tr>`;
  return `<div class="report-doc">
    ${reportHeader('Cash Statement — '+c.person)}
    <div class="report-stats">
      <div class="report-stat accent-green"><div class="rlabel">Total In</div><div class="rval green">${c.totalInFmt}</div></div>
      <div class="report-stat accent-red"><div class="rlabel">Total Out</div><div class="rval red">${c.totalOutFmt}</div></div>
      <div class="report-stat ${c.isNegative?'accent-red':'accent-green'}"><div class="rlabel">Net (Cash in Hand)</div><div class="rval ${c.isNegative?'red':'green'}">${c.amountFmt}</div></div>
    </div>
    <div class="report-section-title">Money In (${c.itemsIn.length})</div>
    <table>
      <thead><tr><th>Date</th><th>Details</th><th class="num">Amount</th></tr></thead>
      <tbody>${itemRows(c.itemsIn, 'amt-in')}</tbody>
    </table>
    <div class="report-section-title">Money Out (${c.itemsOut.length})</div>
    <table>
      <thead><tr><th>Date</th><th>Details</th><th class="num">Amount</th></tr></thead>
      <tbody>${itemRows(c.itemsOut, 'amt-out')}</tbody>
    </table>
    <div class="report-footer">Generated by the Ganesh Pooja Expense Portal · ${escapeHtml(store.settings.committee_name||'')}</div>
  </div>`;
}
function printCustodyDetail(){
  if(!custodyDetailCurrent) return;
  document.getElementById('printReportArea').innerHTML = buildCustodyDetailReportHTML(custodyDetailCurrent);
  window.print();
}
document.getElementById('printCustodyDetailBtn').addEventListener('click', printCustodyDetail);
...
function buildReportHTML(kind){
  ...
  let stats = '';
  let body = '';
  // Cash only -- this is what actually moved through the account and is
  // what Balance is computed from, so the two numbers stay reconcilable.
